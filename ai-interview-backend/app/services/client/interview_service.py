...
from app.services.common.rag import retrieve_questions, fallback_retrieve_by_category
...
class InterviewService:
    ...
    async def start_interview(
        ...
        question_pool = await retrieve_questions(
            db=db,
            skills=skills if skills else [target_position],
            job_type=target_position,
            limit=20
        )

        if question_pool and len(question_pool) >= 3:
            questions = await AIService.generate_questions_with_pool(
            ...
            logger.info(f"RAG 检索到 {len(question_pool)} 道候选题，已生成个性化面试题")
        else:
            ...
            logger.info("RAG 候选题不足，使用 LLM 直接生成面试题")
        ...
        reference_answer = questions[0].get("reference_answer", "")
        key_points = questions[0].get("key_points", [])
    ...
    async def submit_answer(
        ...
        if is_finished:
            ...
            next_reference_answer = questions[next_index].get("reference_answer", "")
            next_key_points = questions[next_index].get("key_points", [])
            ...
            interviewer_msg = InterviewMessage(
                ...
                reference_answer=next_reference_answer if next_reference_answer else None,
                key_points=next_key_points if next_key_points else None
            )
